...
#pragma warning restore 0649
        ...
        [field: SerializeField, Tooltip("Gravity Scalar to make the character feel heavier or lighter as needed.")] public float gravityScale { get; set; } = 1f;
        [SerializeField, Min(0f), Tooltip("Additional air drag.")] private float _airDrag;
        [field: SerializeField, Tooltip("Up to use for the gravity calculations.")] public Transform gravityUpOverride { get; set; }
        [field: SerializeField, Tooltip("The speed at which the character snaps to the gravity orientation.")] public float gravityOrientationSharpness { get; set; } = 20f;
        ...
        {
            get => _airDrag;
            set => _airDrag = Mathf.Max(value, 0f);
        }

        private Vector3 localGravity => gravityScale * (gravityUpOverride != null ? Quaternion.FromToRotation(-Physics.gravity.normalized, gravityUpOverride.up) * Physics.gravity : Physics.gravity);
...
#pragma warning restore 0414 
        ...
        public void UpdatePhase2(float deltaTime)
        {
            ...
            if (gravityOrientationSharpness > 0f)
            {
                var smoothedGravityDir = Vector3.Slerp(CharacterUp, -localGravity.normalized, 1 - Mathf.Exp(-gravityOrientationSharpness * deltaTime));
                _transientRotation = Quaternion.FromToRotation(CharacterUp, smoothedGravityDir) * _transientRotation;
            }
            ...
            {
                BaseVelocity += localGravity * deltaTime;
                BaseVelocity *= 1f / (1f + airDrag * deltaTime);
            }
