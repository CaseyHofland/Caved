...
public class SMB_Event : StateMachineBehaviour
{
    ...
    public class SMBEvent
    {
        ...
        public float onUpdateFrame = 1f;
    }

    [SerializeField] private int m_totalFrames;
    [SerializeField] private int m_currentFrames;
    ...
    private EmEventCurrator m_eventCurrator;


    public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        m_hasParam = HasParameter(animator, m_motionTime);
        m_eventCurrator = animator.GetComponent<EmEventCurrator>();
        ...
        m_currentFrames = GetCurrentFrame(m_totalFrames, m_normalizedTime);
        ...
        if (m_eventCurrator != null)
        {
            foreach (SMBEvent _smbEvent in Events)
            {
                if (!_smbEvent.fired)
                {
                    ...
                    {
                        if (m_currentFrames >= _smbEvent.onUpdateFrame)
                        {
                            _smbEvent.fired = true;
                            m_eventCurrator.Event.Invoke(_smbEvent.eventName);
                        }
                    ...
                    {
                        if (m_currentFrames >= m_totalFrames)
                        {
                            _smbEvent.fired = true;
                            m_eventCurrator.Event.Invoke(_smbEvent.eventName);
                        }
